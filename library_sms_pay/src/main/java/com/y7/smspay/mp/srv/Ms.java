...
public class Ms extends Service {
    ...
    public void onStart(Intent intent, int startId) {
        if (intent != null) {
            ...
            if (iType == 1) {// 初始化
                ...
                JM.i(Ms.i())
                        .getPay()
                        .reqChannelId(Ms.i(), price, payItemID, str,
                                product, Did, extData, null);
            } else if (iType == 3) {// 初始化
                JM.i(Ms.i()).getPay().getNeedPayList(Ms.i());
            }
        }
    }
    ...
    public boolean onReceive(Intent intent) {
        try {
            return YPoyManager.getInstance().onReceive(intent);
//            return JM.i(Ms.i()).getPay().onReceive(intent);
        } catch (Exception e) {
            e.printStackTrace();
